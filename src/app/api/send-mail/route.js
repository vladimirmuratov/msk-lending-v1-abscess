...
export async function POST(request) {
    ...
    const message = {
        ...
        html: ` <p>Имя: ${req?.person}</p>
                ...
                <p>Сообщение пришло с сайта: abscess.mskdoctor.ru</p>`,
    }
    ...
    if(req.email){
        const message2 = {
            from: process.env.ADDRESS_FROM,
            to: [req.email],
            subject: `Вы оставили заявку на звонок на сайте abscess.mskdoctor.ru ${new Date().toLocaleString('ru-RU', {timeZone: 'Europe/Moscow'})}`,
            html: ` <p>Имя: ${req?.person}</p>
                    <p>Дата рождения: ${req?.dateBirth ?? 'Нет'}</p>
                    <p>Телефон: ${req?.phone}</p>
                    <p>Email: ${req?.email ?? 'Нет'}</p>
